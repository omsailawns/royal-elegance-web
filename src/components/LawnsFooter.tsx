  ...
  <footer className="bg-royal text-cream py-16">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        <div>
          <h3 className="font-display text-2xl font-bold gold-text mb-4">OM SAI LAWNS</h3>
          <p className="text-cream/60 text-sm leading-relaxed">Where Celebrations Become Memories. Premium outdoor lawn venue for your special occasions under the open sky.</p>
        </div>
        <div>
          ...
          <div className="space-y-2">
            {["About", "Services", "Gallery", "Contact"].map((link) => (
              <Link key={link} to={`/lawns/${link.toLowerCase()}`} className="block text-cream/60 hover:text-gold-light transition-colors text-sm">{link}</Link>
            ))}
          </div>
        </div>
        <div>
